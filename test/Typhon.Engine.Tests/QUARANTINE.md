...
| `Typhon.Engine.Tests.StorageModeStressTests.Transient_SpawnDestroy_Throughput` | [#708](https://github.com/log2n-io/Typhon/issues/708) | Concurrent Transient spawn hands out **colliding slots** — a thread reads another thread's stamp through an `EntityId` its own `Spawn` returned. Deterministic: 3/3 running the test ALONE. Was `[Ignore("Flaky: throughput threshold … under parallel test load")]`; both halves of that label were wrong, and that is what hid it. | 2026-08-07 |
| `Typhon.Engine.Tests.StorageModeReadWriteTests.DirtyBitmap_ConcurrentSet` | [#709](https://github.com/log2n-io/Typhon/issues/709) | `DirtyBitmap.Set` loses a bit: its `Interlocked.Or` targets the array the thread CAPTURED, which `Grow`/`Snapshot` may already have swapped out of `_bits`. Passes alone (4/4), fails under parallel load. **Not `Sensitive`** — that tier runs a test alone, which for a contention-only race is a guaranteed false green. | 2026-08-07 |
...
| `Typhon.Engine.Tests.ChaosStressTests.CreateDeleteRecreate_RapidLifecycle` | [#695](https://github.com/log2n-io/Typhon/issues/695) | **Livelocks** in the commit publish phase (`BTree.SpinWriteLock`). Re-confirmed on `main` 2026-08-07 — the run needed `--blame-hang` to terminate at all. | 2026-08-07 |
| `Typhon.Engine.Tests.ClusterIndexMatrixTests.MixedPublicationTimings_MutateAndDestroy_LeaveTheIndexAgreeing` | [#711](https://github.com/log2n-io/Typhon/issues/711) | On an archetype whose components publish at **different times** (`SvPlusTransient`+`Commit`, or `VerPlusTransient` under any discipline), a staged key move leaves the secondary index pointing at a slot the entity no longer occupies. Silent for `AllowMultiple`, loud for `Unique`. Deterministic, 12/12 affected cells. The two key-move behaviours in the same fixture narrow via `AxisArchetypes.MixesPublicationTimings` — **delete that predicate and both narrowings when #711 is fixed.** | 2026-08-07 |
...
| `Typhon.Engine.Tests.DifferentialRecoveryOracleTests.PostRecoveryWrite_SurvivesASecondCrash` (3 shapes) | [#712](https://github.com/log2n-io/Typhon/issues/712) | A crash-recovered engine's WAL writer **restarts its LSN sequence at 1**, below the frontier the recovery it just performed replayed — so every commit it then acknowledges collides with an already-used LSN and the next recovery discards it as already-consolidated. Measured: frontier 16, `LastAppendedLsn` 9, `DurableLsn` 16, next open scans 8 / applies **0**, all 12 entities lost. LOG-08's failure mode on the crash path; the clean-reopen half is fixed and green. Found by #705's write-after-recovery axis on its first run. | 2026-08-07 |
| `Typhon.Engine.Tests.AxisArchetypesTests.SvWithUniqueIndex_AfterHardCrash_StillOpens` | [#710](https://github.com/log2n-io/Typhon/issues/710) | A hard crash on a SingleVersion archetype with a **unique** `[Index]` leaves the database **permanently unopenable**: TickFence SV values are legitimately lost (they read back zeroed), so rebuilding a unique index over 64 identical keys throws out of `InitializeArchetypes`. Deterministic. Found by #704's covering array on a cell nobody had written — storage mode × index flavour × crash × discipline. | 2026-08-07 |
