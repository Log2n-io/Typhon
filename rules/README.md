...
| [subscriptions.md](subscriptions.md) | Engine-owned replication | Projection (component values are read only through the archetype's cluster layout — `SUB-01`), Replication State Sizing (cost follows the watched set, never the archetype — `SUB-13`; no managed allocation in the steady state — `SUB-07`), Per-Entity State Lifetime (state follows its entity and never survives slot reuse — `SUB-09`, `[UNBUILT]`), Session Ownership (session state has one writer, the tick; transport threads touch rings, leases and counters — `SUB-05`), Ingress (the inbound ring publishes with release/acquire — `SUB-04`; a command is drained once, visible for exactly its tick, in per-session order — `SUB-08`) |
