...
The one exception is [`Transient`](xref:concept-storage-mode) components — RAM-only scratch by design, never paged to disk. Defaults are intentionally *small* (a 2 MB cache) so development exercises the cache machinery; size it up for real workloads and let the engine self-manage eviction.
