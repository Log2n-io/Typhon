...
> 📌 **Precision:** both precisions index. The `f32` boxes (`AABB2F` / `AABB3F`) suit most worlds; the double-precision boxes (`AABB2D` / `AABB3D`, and the f64 bounding spheres) are fully supported on the live spatial path — grid bucketing, cluster bounds and narrowphase all handle them — for a world whose coordinates outgrow `float`. In both cases the cluster bounds are stored as `f32` boxes relative to their cell.
