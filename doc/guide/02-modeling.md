...
> ⚠️ **One placement constraint worth knowing early.** An archetype with any SingleVersion or Transient slot is stored as a **cluster** (a packed SoA layout — that's what makes the parallel tick loop fast), and cluster archetypes route their indexes through one fixed-stride segment that can't hold a 64-byte `String64` key. So a `String64` **unique index** wants a purely-`Versioned` archetype. In the sample, `Guild.Name` and `Recipe.Name` are `[Index] String64` on Versioned-only archetypes, while `Player` — which is cluster-eligible thanks to its SV position and Transient session — uses a `long AccountId` as its unique key and leaves `Name` unindexed.
