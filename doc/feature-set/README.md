...
| &nbsp;&nbsp;↳ Commit Discipline | Zero-loss, atomic writes on Typhon's cheapest component layout (SingleVersion) without paying for an MVCC revision chain. | ✅ Implemented | 🟣 Advanced | [→](Durability/durability-modes/committed-discipline.md) |
| Write-Ahead Log (WAL v2 logical records) | The single source of durability truth: logical (EntityId, slot) records, one codec, a sequential CRC-chained log. | ✅ Implemented | 🟣 Advanced | [→](Durability/wal-v2.md) |
...
| &nbsp;&nbsp;↳ Commit Discipline | Zero-loss, atomic commits on the SingleVersion layout — without paying for a Versioned revision chain. | ✅ Implemented | 🟣 Advanced | [→](Ecs/storage-modes/storage-mode-committed.md) |
...
| &nbsp;&nbsp;↳ Per-Transaction Durability Override | Escalate one critical operation to zero-loss durability — via a dedicated `Immediate` UoW / side-transaction — without raising the surrounding batch's mode (a `DurabilityOverride` enum was sketched for this but never committed). | ✅ Implemented | 🟣 Advanced | [→](Transactions/durability-modes/durability-override-escalation.md) |
...
| SingleVersion Commit Discipline (TickFence / Commit) | Per-transaction knob, orthogonal to DurabilityMode, that picks how a SingleVersion write becomes durable. | ✅ Implemented | 🔵 Core | [→](Transactions/commit-discipline/README.md) |
| &nbsp;&nbsp;↳ TickFence Discipline (Default) | The default, lowest-cost SingleVersion write — durable at the next tick fence, not at commit. | ✅ Implemented | 🔵 Core | [→](Transactions/commit-discipline/commit-discipline-tickfence.md) |
| &nbsp;&nbsp;↳ Commit Discipline (Variant-A Staging) | Atomic, zero-loss SingleVersion writes — durable and visible together at Commit(), with no revision chain. | ✅ Implemented | 🟣 Advanced | [→](Transactions/commit-discipline/commit-discipline-commit.md) |
...
| Typed Event Queues | Multi-producer per-worker-segment queues for inter-system signalling within a tick; any number of parallel chunk workers may push concurrently via `ctx.Writer(queue)`, enabling reactive cascade chains that early-out cheaply when dormant. | ✅ Implemented | 🟣 Advanced | [→](Runtime/typed-event-queues.md) |
...
| &nbsp;&nbsp;↳ Options Validation | Engine and storage options are range-checked at DI resolution by two IValidateOptions\<T\> implementations, so a bad knob throws OptionsValidationException at startup rather than surfacing mid-open. | ✅ Implemented | 🟣 Advanced | [→](Hosting/engine-options-configuration/options-validation.md) |
