...
| &nbsp;&nbsp;↳ Options Validation Hooks | AddOptions\<T\>().Validate(...) is wired on every options type but its predicate is a no-op stub today; DatabaseEngineOptionsValidator range-checks the wired Resources/WAL knobs at DI resolution, and PagedMMFOptions.IsValid is a directly-callable fail-fast check. | 🚧 Partial | 🟣 Advanced | [→](Hosting/engine-options-configuration/options-validation-stubs.md) |
