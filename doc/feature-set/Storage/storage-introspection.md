...
- `RunStorageIntegrityCheck` is the canary for three independent invariant classes: a **popcount canary** (occupancy bitmap set-bit count vs. every page claimed by a segment, its directory-extension pages, reserved roots, occupancy reserves, and directory twins), **chunk-segment capacity** (`AllocatedChunkCount + FreeChunkCount == ChunkCapacity` per chunk-based segment), and the **cluster MVCC visibility summary** (each cluster's `ClusterMaxBornTsn` / `ClusterMaxDiedTsn` recomputed from the archetype's EntityMap — a summary claiming more visibility than its entities justify would let the scan skip its per-entity visibility probe and emit a phantom). A non-empty `Issues` list is a hard structural/durability bug, not a soft warning — `IsHealthy` is the single assertion callers need.
- The visibility-summary check is **one-directional by design**: only a summary claiming *more* visibility than the data supports is reported. The summary is a conservative approximation — claiming less merely costs a probe and stays correct — so an over-pessimistic entry is a legal state, never a finding. `VisibilitySummaryClustersChecked` reports how many clusters were recomputed, distinguishing "found nothing" from "looked at nothing".
- The visibility-summary pass is **O(entities)**, unlike the O(pages) storage passes — the audit's cost on a large database is dominated by it.
...
- [ClusterVisibilitySummaryIntegrityTests](https://github.com/Log2n-io/Typhon/blob/main/test/Typhon.Engine.Tests/Data/ECS/ClusterVisibilitySummaryIntegrityTests.cs) — the visibility-summary pass: clean on a healthy engine, fires on both unsound directions, silent on the pessimistic one
