...
Opening a database acquires two independent locks. The OS-level one is the real guarantee: the data file handle is opened with `FileShare.Read`, so a second process requesting read-write access is rejected by the OS kernel (mandatory on Windows; advisory-only on POSIX). The advisory one is a JSON sidecar, `db.lock`, written inside the database's `{name}.typhon` bundle directory with the owning process's PID, machine name, start time, whether it will yield the database on request, and (when configured) the live-profiler TCP endpoint — its primary job is to turn an opaque OS sharing-violation into a `DatabaseLockedException` that names the culprit; the profiler endpoint lets an observer that already has the database path (e.g. the Workbench opening a bundle the application holds) discover where to watch without the user retyping an address they never chose. The lock file is acquired before the data file is opened and removed on `Dispose()`; a dead owner's lock is detected (PID not running) and cleared automatically rather than blocking the new open. Deleting a database file polls for NTFS's deferred directory-entry removal so a delete-then-recreate of the same name doesn't race the filesystem.
