...
| [SingleVersion Commit Discipline (TickFence / Commit)](commit-discipline/README.md) | Per-transaction knob, orthogonal to `DurabilityMode`, that picks how a `SingleVersion` write becomes durable | ✅ Implemented | 🔵 Core |
| &nbsp;&nbsp;↳ [TickFence Discipline (Default)](commit-discipline/commit-discipline-tickfence.md) | The default, lowest-cost `SingleVersion` write — durable at the next tick fence, not at commit | ✅ Implemented | 🔵 Core |
| &nbsp;&nbsp;↳ [Commit Discipline (Variant-A Staging)](commit-discipline/commit-discipline-commit.md) | Atomic, zero-loss `SingleVersion` writes — durable and visible together at `Commit()`, with no revision chain | ✅ Implemented | 🟣 Advanced |
