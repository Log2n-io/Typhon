...
| [Telemetry Flags Reference (generated)](telemetry-flags-reference.md) | The complete list of every gate flag — config key, default, and what each enables — plus a copy-paste `typhon.telemetry.json` template. Generated from `TelemetryConfig.cs` by `scripts/gen-telemetry-reference.py` and CI-guarded against drift | ✅ Implemented | 🟢 Start Here |
