...
| `QueryEfficiencyTolerance` | `0.1` | How far above the best candidates per hit an archetype's range queries may sit before its maintenance gets the whole `ReclusterBudgetMs`; next to nothing is granted at the best. `0` grants the configured budget every tick |
