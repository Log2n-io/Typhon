...
| [Realms — Several Worlds in One Engine](realms.md) | Isolated worlds (planets, interiors, space, instanced dungeons) in one engine: each realm owns its grid and cell size, is queried alone, and sleeps or runs divided when nobody watches it | ✅ Implemented | 🟣 Advanced |
| [Spatial Grid Configuration & Tier Control](spatial-grid-config.md) | Per-realm grid sizing (realm 0 by default) plus the per-cell `SimTier` control surface for multi-resolution simulation | ✅ Implemented | 🔵 Core |
