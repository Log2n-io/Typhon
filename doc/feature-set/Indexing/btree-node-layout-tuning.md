...
- Capacity scales inversely with key width: `L16BTree`=38, `L32BTree`=29, `L64BTree`=19 keys/node; `String64BTree` stays at 4 because a 64-byte key already consumes most of the budget. `String64BTree` is also the one variant that does **not** fit the 256-byte node: `IndexString64Chunk` is **356 bytes** (4 keys × 64 B, plus a 64 B B-link `HighKey`), so its segment stride is 356 and its nodes span six cache lines rather than four. See [B+Tree Key-Size Variants](./btree-key-variants.md) for the full type-to-variant mapping.
