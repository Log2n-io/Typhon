...
| [Element-Precise Secondary Indexes for MVCC](versioned-secondary-indexes.md) | `AllowMultiple` index mutations address the entity's own element inside the key's buffer, so a value change or delete never strands the entity between keys nor evicts its siblings | ✅ Implemented | 🟣 Advanced |
