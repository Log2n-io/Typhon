...
- Thread IDs are stored in exactly 16 bits everywhere, but the usable range is not uniform: `AccessControl` (bits 32–47 of its `ulong`) and `ResourceAccessControl` (bits 8–23 of its `int`) reach 65,535, while `AccessControlSmall` holds the id in bits 16–31 of a *signed* `int` — a higher id would set the sign bit — and caps at **32,767** simultaneously-live threads. Design to 32,767 — still far past 500+ core servers, and managed thread ids are recycled on thread death.
