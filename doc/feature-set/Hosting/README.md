...
| &nbsp;&nbsp;↳ [Options Validation](engine-options-configuration/options-validation.md) | Engine and storage options are range-checked at DI resolution by two `IValidateOptions<T>` implementations, so a bad knob throws `OptionsValidationException` at startup rather than surfacing mid-open | ✅ Implemented | 🟣 Advanced |
