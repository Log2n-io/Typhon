...
**Exception:** a `SingleVersion` component written under `CommitDiscipline.Commit` (an escalation from the `TickFence` default, selected per transaction) *does* get commit-scoped visibility, all-or-nothing atomicity, O(1) `Rollback`, and commit-WAL durability — without the revision chain or snapshot isolation Versioned pays for. See [06-ecs §8](06-ecs.md) ("`CommitDiscipline` — a second, orthogonal axis on SingleVersion").
