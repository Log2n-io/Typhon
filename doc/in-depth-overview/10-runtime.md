...
- **`OnTickAborted`** — fires at most once, only under `SystemExceptionPolicy.AbortTickAndStop` ([§6](#6-workers)), on the TickDriver thread once all systems have run or been skipped. "Drained" is not "finished": the handler is called *inside* the tick — the tick's own end-of-tick accounting (including the `CurrentTickNumber` increment in the telemetry finalizer) still runs after the handler returns. `CurrentTickNumber` can therefore advance by one more after this handler exits. The handler receives the `TickOutcome` naming the failing system and its exception.
...
- **`FatalStop()`** — stops the runtime **without** running `OnShutdown`. This is the path for a host reacting to `OnTickAborted`, where the `Immediate`-durability shutdown transaction is precisely what you don't want to run on a simulation whose systems only partly ran. Because `OnTickAborted` fires inside the tick, a `FatalStop()` call from the handler returns into the still-running tick; `CurrentTickNumber` advances once more when that tick's accounting completes. `Dispose()` is the quiescence point — it joins the timer thread, after which nothing can post further accounting.
