...
**On the "PK fallback":** When `SelectPrimaryStream` cannot narrow a range — in practice an all-`!=` predicate, since `NotEqual` is excluded from range selection — `BuildPlanWithPrimarySelection` now calls `SelectFullScanStream`, which picks any indexed field referenced by the predicate and scans its full type range, leaving every evaluator to filter. For the typical NE-only case on a non-cluster archetype the plan is therefore executable with a non-negative `PrimaryFieldIndex`. `PrimaryFieldIndex = -1` and a **non-executable plan** are still produced in two remaining cases: OrderBy PK (`orderByFieldIndex == -1`, because no secondary index reproduces PK order) and the degenerate case where no evaluator carries a usable index (unreachable in practice for non-cluster archetypes — `WhereField` rejects non-indexed fields). For **non-cluster archetypes** in either case, `PipelineExecutor.ExecuteCore` short-circuits when `UsesSecondaryIndex` is false; `Count` returns 0 directly. The PK B+Tree that the historical full-table-scan fallback used was removed; the engine relies on every query reaching a secondary index. Cluster-storage scans (ordered and unordered) handle a `-1` plan separately: ordered scans use `ExecuteOrderedClustered` with the OrderBy field's per-archetype B+Tree directly; unordered scans use `ScanPerArchetypeBTree` via `ExecuteTargeted` — both scan the per-archetype B+Trees and return correct results. `Count` for cluster archetypes routes through `ExecuteTargeted().Count` (`EcsQuery.cs:2176`) and is likewise never 0 due to plan shape.
