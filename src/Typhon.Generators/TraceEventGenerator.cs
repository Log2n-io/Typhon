...
public class TraceEventGenerator : IIncrementalGenerator
{
    ...
    {
        // The [TraceEvent] / [Optional] / [BeginParam] attributes and the TraceEventShape enum are declared as real
        // source in Typhon.Engine (Profiler/public/TraceEventAttributes.cs), NOT synthesized here. Emitting them from
        // post-init output injected a duplicate copy into every project that references this generator as an analyzer
        // (all of which also reference Typhon.Engine), producing CS0436 duplicate-type warnings. Real source is visible
        // to generators, so ForAttributeWithMetadataName below resolves against the engine declaration directly.
    ...
    /// </summary>
    private static readonly HashSet<string> ReservedDtoBaseNames = new(System.StringComparer.Ordinal)
    {
    ...
    /// </summary>
    private static string EmitPolymorphismRegistration(ImmutableArray<TraceEventModel> models)
    {
        ...
        var ordered = new List<TraceEventModel>();
        foreach (var m in models)
        {
    ...
    /// </list>
    /// </summary>
    private static string EmitTopLevelDispatch(ImmutableArray<TraceEventModel> models)
    {
