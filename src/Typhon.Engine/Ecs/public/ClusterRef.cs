...
public unsafe ref struct ClusterRef<TArch> where TArch : class
{
    ...
    public Span<T> GetSpan<T>(Comp<T> comp) where T : unmanaged
    {
        ...
        CheckConfig.Require(CheckConfig.Enabled, (_meta.VersionedSlotMask & (1 << slot)) == 0,
    ...
    public ref T Get<T>(Comp<T> comp, int slotIndex) where T : unmanaged
    {
        var slot = _meta.GetSlot(comp._componentTypeId);
        CheckConfig.Require(CheckConfig.Enabled, (_meta.VersionedSlotMask & (1 << slot)) == 0,
            $"Get on Versioned component bypasses revision chain. Use OpenMut+Write for writes.");
    ...
    {
        var slot = _meta.GetSlot(comp._componentTypeId);
        CheckConfig.Require(CheckConfig.Enabled, (_meta.VersionedSlotMask & (1 << slot)) == 0, $"WriteSpatial on Versioned component bypasses revision chain.");
        CheckConfig.Require(CheckConfig.Enabled, _state != null && _state.SpatialSlot.HasSpatialIndex && _state.SpatialSlot.Slot == slot,
            $"WriteSpatial requires the archetype's spatial-indexed component (the one marked [SpatialIndex]). For non-spatial fields, use GetSpan or Get.");
