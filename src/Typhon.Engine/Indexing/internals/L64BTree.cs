...
internal abstract class L64BTree<TKey, TStore> : BTree<TKey, TStore> where TStore : struct, IPageStore where TKey : unmanaged
{
    unsafe public class L64NodeStorage : BaseNodeStorage
    {
        ...
        public override KeyValueItem GetItem(NodeWrapper node, int index, bool adjust, ref ChunkAccessor<TStore> accessor)
        {
            ...
            return new KeyValueItem(Unsafe.As<long, TKey>(ref chunkKey), chunk.Values[i]);
        }
        ...
        public override void PushFirst(NodeWrapper node, KeyValueItem item, ref ChunkAccessor<TStore> accessor)
        {
            ...
            chunk.Keys[start] = Unsafe.As<TKey, long>(ref item.Key);
        ...
        {
            var i = adjust ? Index64Chunk.Adjust(chunk.Start + index) : index;
            chunk.Keys[i] = Unsafe.As<TKey, long>(ref item.Key);
